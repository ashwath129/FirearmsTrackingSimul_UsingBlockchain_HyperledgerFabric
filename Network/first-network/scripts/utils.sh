...
PEER0_ORG1_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/retail.state.com/peers/peer0.retail.state.com/tls/ca.crt
PEER0_ORG2_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/firearmsManu.state.com/peers/peer0.firearmsManu.state.com/tls/ca.crt
PEER0_ORG3_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/police.state.com/peers/peer0.police.state.com/tls/ca.crt
PEER0_ORG4_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/ncis.state.com/peers/peer0.ncis.state.com/tls/ca.crt
...
setGlobals() {
  ...
  if [ $ORG -eq 1 ]; then
    ...
    CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/firearmsManu.state.com/users/Admin@firearmsManu.state.com/msp
    CORE_PEER_ADDRESS=peer0.firearmsManu.state.com:9051

  elif [ $ORG -eq 3 ]; then
    CORE_PEER_LOCALMSPID="policeMSP"
    CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_ORG3_CA
    CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/police.state.com/users/Admin@police.state.com/msp
    CORE_PEER_ADDRESS=peer0.police.state.com:10051
	
  elif [ $ORG -eq 4 ]; then
    CORE_PEER_LOCALMSPID="ncisMSP"
    CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_ORG4_CA
    CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/ncis.state.com/users/Admin@ncis.state.com/msp
...
installChaincode() {
  ...
  peer chaincode install -n facc -v ${VERSION} -l ${LANGUAGE} -p ${CC2_SRC_PATH} >&log.txt
  res=$?
  set +x
  cat log.txt
...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode instantiate -o orderer.state.com:7050 -C $CHANNEL_NAME -n mycc -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["init","a","100","b","200"]}' -P "AND ('retailMSP.peer','firearmsManuMSP.peer')" >&log.txt
    
	peer chaincode instantiate -o orderer.state.com:7050 -C $CHANNEL_NAME -n facc -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["initGun",""]}' >&log.txt
	
	peer chaincode instantiate -o orderer.state.com:7050 -C $CHANNEL_NAME -n facc -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["initCust",""]}' >&log.txt
    ...
    set -x
    peer chaincode instantiate -o orderer.state.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -l ${LANGUAGE} -v 1.0 -c '{"Args":["init","a","100","b","200"]}' -P "AND ('retailMSP.peer','firearmsManuMSP.peer')" >&log.txt
    peer chaincode instantiate -o orderer.state.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n facc -l ${LANGUAGE} -v 1.0 -c '{"Args":["initGun",""]}'>&log.txt
    peer chaincode instantiate -o orderer.state.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n facc -l ${LANGUAGE} -v 1.0 -c '{"Args":["initCust",""]}'>&log.txt  	
   res=$?
    set +x
  fi
  cat log.txt
...
upgradeChaincode() {
  ...
  set -x
  peer chaincode upgrade -o orderer.state.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -v 2.0 -c '{"Args":["init","a","90","b","210"]}' -P "AND ('retailMSP.peer','firearmsManuMSP.peer')"
  res=$?
  set +x
  cat log.txt
...
chaincodeInvoke() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode invoke -o orderer.state.com:7050 -C $CHANNEL_NAME -n mycc $PEER_CONN_PARMS -c '{"Args":["invoke","a","b","10"]}' >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode invoke -o orderer.state.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc $PEER_CONN_PARMS -c '{"Args":["invoke","a","b","10"]}' >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
